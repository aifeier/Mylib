package com.cwf.app.cwflibrary.widget.gesture;
...
import com.cwf.app.cwflibrary.widget.gesture.GestureLockView.Mode;
...
//	private int[] mAnswer = { 0, 1, 2, 5, 8 };
	...
	public GestureLockViewGroup(Context context, AttributeSet attrs, int defStyle) {
		...
		for (int i = 0; i < n; i++) {
			...
			if(attr == R.styleable.GestureLockViewGroup_color_no_finger_outer_circle){
				mNoFingerOuterCircleColor = a.getColor(attr, mNoFingerOuterCircleColor);
			}else
			if(attr == R.styleable.GestureLockViewGroup_color_finger_on){
				mFingerOnColor = a.getColor(attr, mFingerOnColor);
			}else
			if(attr == R.styleable.GestureLockViewGroup_color_finger_up ){
				mFingerOnColor = a.getColor(attr, mFingerOnColor);
			}else
			if(attr == R.styleable.GestureLockViewGroup_count){
				mCount = a.getInt(attr, 3);
			}else
			if(attr == R.styleable.GestureLockViewGroup_tryTimes){
...
//			reset();
	...
	private boolean checkAnswer() {
		String t = MD5Util.md5Encrypt(mResult);
		mOnGestureLockViewListener.onGestureResult(t);
		if (mAnswer.length()!= t.length())
			return false;
		if(mAnswer.equals(t))
			return true;
		else return false;
/*
		for (int i = 0; i < mAnswer.length; i++) {
			Log.e(TAG, "mAnswer" + mAnswer[i] + "mChoose" + mChoose.get(i));
